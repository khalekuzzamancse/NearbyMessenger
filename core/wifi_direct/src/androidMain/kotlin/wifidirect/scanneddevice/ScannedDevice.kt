...
/**
 * * if it is the group owner then will get the client list,otherwise the list is empty though is it is connected.
 * * if the device is not group owner then its connected with only,a single device that is the group owner.
 * * group != null ,if there are connected devices/network
 */

/**
 * Using data class to use the copy method and changing.
 */
