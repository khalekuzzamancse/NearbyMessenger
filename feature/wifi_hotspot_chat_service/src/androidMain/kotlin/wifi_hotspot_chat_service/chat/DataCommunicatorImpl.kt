...
class DataCommunicatorImpl(
    ...
    override suspend fun sendMessage(msg: SendAbleMessage): Result<Unit> {
        val isWifiConnected=WiFiHotspotFactory.receiver.isWifiConnected()
        val thisDeviceNotHotspotOwner=WiFiHotspotFactory.receiver.getHotspotDefaultGateway()!=null
        if (!isWifiConnected&&thisDeviceNotHotspotOwner)
            return  Result.failure(Throwable("You are not connect to a Hotspot"))
